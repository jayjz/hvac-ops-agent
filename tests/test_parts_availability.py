...
async def test_flags_missing_parts_and_recommends_reorder():
    ...
    mock_mongodb = MagicMock()
    mock_mongodb.get_low_inventory.return_value = [{"sku": "FILTER-01", "quantity": 5, "name": "Air Filter"}]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("zero_stock", [RequiredPart(sku="PUMP-001", name="Main Pump", quantity=5)], [{"sku": "PUMP-001", "quantity": 0}], False, 0.0, 1, "high"),
        ("partial", [RequiredPart(sku="BELT-01", name="Drive Belt", quantity=3)], [{"sku": "BELT-01", "quantity": 1}], False, 0.33, 1, "high"),
        ("high_shortfall", [RequiredPart(sku="COMP-01", name="Compressor", quantity=10)], [{"sku": "COMP-01", "quantity": 1}], False, 0.1, 1, "high"),
    ],
)
@pytest.mark.asyncio
async def test_edge_cases(test_case, required_parts, low_inventory, expected_available, expected_score, expected_reorder_count, expected_urgency):
...
async def test_exception_handling():
    ...
    result: AgentResult = await agent.execute(
        ...
        payload={
            ...
            "required_parts": [RequiredPart(sku="TEST-01", name="Test Part", quantity=2)],
        }
    )
