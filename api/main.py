...
class PMJobRequest(BaseModel):
    ...
    goals: List[str] = Field(
        ...,
        example=[
            "Forecast inventory for upcoming heat pump installs",
            "Optimize technician schedules for next week",
        ],
    )
...
async def start_modernization_job(
    request: PMJobRequest, background_tasks: BackgroundTasks
):
    """
    ...
    jobs[job_id] = JobStatus(
        job_id=job_id, status="PENDING", details="Job accepted and queued."
    )
