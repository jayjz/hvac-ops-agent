...
class SchedulerOptimizerAgent(BaseAgent):
    ...
    async def execute(self, context: Dict[str, Any]) -> ScheduleOptimizationResult:
        # Pull jobs from context or Mongo (use existing mongodb_tools pattern)
        jobs = context.get("jobs", [])  # or await mongodb_tools.get_upcoming_jobs()
        ...
        return ScheduleOptimizationResult(
            optimized_jobs=[j[0] for j in optimized_jobs],
